...
def add_convolutional_block(model, filters=256, kernel_size=[3,3], c_reg=0.0001):
    ...
    model = Conv2D(filters= filters,
             ...
             activation='linear',
             use_bias=False,
             kernel_regularizer=regularizers.l2(c_reg))(model)
    model = BatchNormalization()(model)
    model = LeakyReLU()(model)

    return model

def add_residual_block(input_layer, filters=256, kernel_size=[3,3], c_reg=0.0001, name=None):
...
def add_policy_head(model, output_shape, filters=2, kernel_size=[1, 1], c_reg=0.0001):
    ...
    model = Flatten()(model)
    model = Dense(output_shape,
              activation='linear',
              kernel_regularizer=regularizers.l2(c_reg),
              use_bias=False,
              name='policy_head')(model)
    return model
...
def add_policy_value_heads(
    model, input_layer,
    policy_output, policy_filters=2,policy_kernel=[1, 1], policy_reg=0.0001,
     value_dense_shape=4, value_filters=1, value_kernel=[1, 1], value_reg=0.0001):
